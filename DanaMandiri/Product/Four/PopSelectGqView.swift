...
class PopSelectGqView: UIView {
    ...
    var model: sipirangeularModel? {
        didSet {
            ...
            nameLabel.text = model.evidenceish ?? ""
    ...
    lazy var tableView: UITableView = {
        ...
        tableView.register(PopSelectInfoViewCell.self, forCellReuseIdentifier: "PopSelectGqViewCell")
...
extension PopSelectGqView: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return model?.archile?.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let listModel = model?.archile?[indexPath.row]
        let cell = tableView.dequeueReusableCell(withIdentifier: "PopSelectGqViewCell", for: indexPath) as! PopSelectInfoViewCell
